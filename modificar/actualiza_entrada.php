...
$actualizaquery=$base->query("SELECT * FROM tempentrada WHERE codigo_pedido= $factura AND estado='Pendiente'")->fetchAll(PDO::FETCH_OBJ);
foreach ($actualizaquery as $actualizatabla) {
 $facturaactua=$actualizatabla->codigo_pedido;

$actualizada="UPDATE tempentrada SET  estado='Procesado'
...
$busquedavalores=$base->query("SELECT * FROM tempentrada WHERE codigo_pedido='$factura'")->fetchAll(PDO::FETCH_OBJ);
