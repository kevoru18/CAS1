...
$registros=$base->query("SELECT DISTINCT codigo_pedido FROM  tempentrada WHERE estado='Pendiente' AND codigo_pedido=$ide ")->fetchAll(PDO::FETCH_OBJ);
foreach($registros as $fechaproveedor ):
    $fechaproveedor1= $fechaproveedor->codigo_pedido;
    $fechapendientes=$base->query("SELECT DISTINCT * FROM  tempentrada WHERE estado='Pendiente' AND codigo_pedido= $fechaproveedor1 ")->fetchAll(PDO::FETCH_OBJ);
    $fechapendientes1=$base->query("SELECT DISTINCT fecha_creacion_factura, codigo_pedido, oficina, nombre_proveedor FROM  tempentrada WHERE estado='Pendiente' AND codigo_pedido= $fechaproveedor1 ")->fetchAll(PDO::FETCH_OBJ);
    ...
    $codigoproveed=$fechaproveedor1->oficina;
...
<label for="tipo_buscar" class="control-label">Fecha de la Transacción:</label>
...
<label for="tipo_buscar" class="control-label">Descripción de la Entrada:</label>
...
$pendientes=$base->query("SELECT DISTINCT * FROM  tempentrada WHERE estado='Pendiente' AND codigo_pedido= $factura_pendiente3  ")->fetchAll(PDO::FETCH_OBJ);
...
<td><input type="text" class='form-control pull-center' style='min-width: 80px;' disabled id='cantidad-prodtemp' value=<?php echo $posibles->cantidad?> onkeyup=' calcula_monto();' onclick='calcula_monto(); '></td>
