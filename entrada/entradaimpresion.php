...
$registros=$base->query("SELECT DISTINCT codigo_pedido FROM  tempentrada WHERE estado='Procesado' AND codigo_pedido=$factura ")->fetchAll(PDO::FETCH_OBJ);
foreach($registros as $fechaproveedor ):
  $totalentrada=0;
  $totalentradacantidad=0;
  
  $fechaproveedor1= $fechaproveedor->codigo_pedido;
  $fechapendientes1=$base->query("SELECT DISTINCT fecha_creacion_factura, codigo_pedido,nombre_proveedor FROM  tempentrada WHERE estado='Procesado' AND codigo_pedido= $fechaproveedor1 ")->fetchAll(PDO::FETCH_OBJ);
...
foreach($fechapendientes1 as $fechaproveedor1 ):
    ...
    $pdf->Write(5,utf8_decode('En la presente se desglosa los artículos que estan ingresando al inventario de esta sucursal con orden de entrada No.'. $codigo_factura.'. Con fecha '.$fecha.'.'));
    $pdf->SetY(75);
...
foreach($registros as $factura_pendiente2 ):
  ...
  $pendientes=$base->query("SELECT DISTINCT * FROM  tempentrada WHERE estado='Procesado' AND codigo_pedido= $factura_pendiente3  ")->fetchAll(PDO::FETCH_OBJ);
