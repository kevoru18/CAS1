...
if ($permiso==="Contador") {
  header("location:salidaoficina.php");
...
function obtenerNuevoId(){
  include('../Conection/conexion.php');
  $codigonuevaorden=$base->query("SELECT MAX(codigo_pedido)+ 1 as nuevocodigo FROM tempsalidas")->fetchAll(PDO::FETCH_OBJ);    
...
<head>
  ...
  <title>CAS COMIXMUL - Salidas</title>
   ...
   <li class="nav-item">
          ...
          <div id="collapsePagessalida" class="collapse" aria-labelledby="headingPages" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
              ...
              <!--<a class="collapse-item" href="login.html">Login</a>-->
              <a class="collapse-item" href="salida.php">Oficina</a>
...
<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800">Salida.</h1>
<p class="mb-4"><?php echo $_SESSION["nombre"]?></p>

<!-- DataTales Example -->
<div class="card shadow mb-4">
  <div class="card-header py-3">
  <button id="btnNuevo" type="button"  class="btn btn-success btn-lg active btn btn-sm"  >Nueva salida <i class="fas fa-plus-circle"></i></button>
  <br><br>
    <h6 class="m-0 font-weight-bold text-primary">Salida en lista</h6>
  ...
  <?php
$registros=$base->query("SELECT DISTINCT  codigo_pedido FROM  tempsalidas WHERE estado='Pendiente' ORDER BY Id")->fetchAll(PDO::FETCH_OBJ);
    foreach($registros as $persona):

$factura_pendiente= $persona->codigo_pedido;
$registros1=$base->query("SELECT *, SUM(costoU*cantidad) AS monto FROM tempsalidas WHERE codigo_pedido='$factura_pendiente'")->fetchAll(PDO::FETCH_OBJ);  
$monto1=0;
    ...
    <div class="modal-content">
      ...
      <form id="mAgregar">
        <div class="modal-body ">
          <label>Salida #</label><input type="text" value='<?php echo  obtenerNuevoId();?>'  class="form-control" list="factura_pedido" id="factura_pedido" disabled >
          <label>Descripción de la salida</label>
...
$.ajax({
  ...
  <script src="../js/cas/salida.js" type="text/javascript"> </script>
