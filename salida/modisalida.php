...
$registros=$base->query("SELECT DISTINCT codigo_pedido FROM  tempsalidas WHERE estado='Pendiente' AND codigo_pedido=$ide ")->fetchAll(PDO::FETCH_OBJ);
foreach($registros as $fechaproveedor ):
    $fechaproveedor1= $fechaproveedor->codigo_pedido;
    $fechapendientes=$base->query("SELECT DISTINCT * FROM  tempsalidas WHERE estado='Pendiente' AND codigo_pedido= $fechaproveedor1 ")->fetchAll(PDO::FETCH_OBJ);
    $fechapendientes1=$base->query("SELECT DISTINCT fecha_creacion_factura, codigo_pedido, nombre_proveedor FROM  tempsalidas WHERE estado='Pendiente' AND codigo_pedido= $fechaproveedor1 ")->fetchAll(PDO::FETCH_OBJ);
...
<label for="tipo_buscar" class="control-label">Descripción de la Salida:</label>
...
$pendientes=$base->query("SELECT DISTINCT * FROM  tempsalidas WHERE estado='Pendiente' AND codigo_pedido= $factura_pendiente3  ")->fetchAll(PDO::FETCH_OBJ);
