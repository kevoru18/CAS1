...
foreach($fechapendientes1 as $fechaproveedor1 ):
    ...
    $pdf->Write(5,utf8_decode('Mediante la presente la Cooperativa Mixta Mujeres Unidas Limitada (COMIXMUL) le esta solicitando a la empresa '. $proveedor1.' la compra de los siguientes artículos'));
    $pdf->SetY(75);
...
$pdf->Cell(54, 10, 'Precio Unitario',0,0,'R');
...
foreach($registros as $factura_pendiente2 ):
  ...
  $pendientes=$base->query("SELECT DISTINCT codigo_producto, costoU, descripcion_producto,SUM(cantidad) AS cant, SUM(costo_total) AS sumatoria FROM  temp WHERE estado='Procesado' AND codigo_pedido= $factura_pendiente3 AND codigo_producto=$codigodeprod ")->fetchAll(PDO::FETCH_OBJ);
...
for ($i=0; $i < 15; $i++) { 
  ...
  $pdf->SetX($x);
  $pdf->SetX($y+40);
  $pdf->Cell(15, -5, number_format(round($costo), 2, '.', ' ').Lps,0,0,'R');
  $x=$x+$i;
...
$pdf->Ln(5);
$pdf->SetFont('Courier', 'B', 14);
$pdf->Write(5,utf8_decode('Especificaciones:'));$pdf->Ln(5);
$pdf->Cell(180,0,'','T');
$pdf->SetFont('Courier', '', 14);
$pdf->Write(5,utf8_decode('FORMA DE PAGO 100%'));$pdf->Ln(5);

$pdf->Cell(180,0,'','T');
$pdf->SetFont('Courier', '', 14);
$pdf->Write(5,utf8_decode('TIEMPO DE ENTREGA:CONTRA ENTREGA'));$pdf->Ln(5);
$pdf->Cell(180,0,'','T');
...
$pdf->SetFont('Courier', '', 14);
$pdf->setX(25);
$pdf->Write(5,utf8_decode('Elaborado por'));
$pdf->setX(125);
$pdf->Write(5,utf8_decode('Autorizado por'));
